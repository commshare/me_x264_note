...
/* options that are used by only some demuxers 某些demuxer需要这些选项*/
typedef struct
{
    char *index_file;
    char *format;
    char *resolution; //分辨率
    char *colorspace;  //颜色空间
    int bit_depth; //位深
    char *timebase;  //时间戳为啥是c风格字符串呢?
    int seek;
    int progress;
...
/* properties of the source given by the demuxer demuxer所提供的输入源的属性*/
typedef struct
{
